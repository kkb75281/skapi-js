...
export default class Skapi {
    ...
    constructor(
        ...
        if (!sessionStorage) {
            throw new SkapiError("Web browser API is not available.", {
                code: "NOT_SUPPORTED",
            });
        }
        ...
        if (sessionStorage.getItem("__skapi_kiss") !== "kiss") {
            alert(
                "Session storage is disabled. Please enable session storage."
            );
            throw new SkapiError(
                "Session storage is disabled. Please enable session storage.",
                { code: "SESSION_STORAGE_DISABLED" }
            );
        }
        ...
        if (typeof service !== "string" || typeof owner !== "string") {
            ...
            throw new SkapiError(
                '"service" and "owner" should be type <string>.',
        ...
        if (!service || !owner) {
            alert("Service ID or Owner ID is invalid.");
            throw new SkapiError('"service" and "owner" is required', {
                code: "INVALID_PARAMETER",
            });
        }
        ...
        if (
            options?.eventListener?.onLogin &&
            typeof options.eventListener.onLogin === "function"
        ) {
        ...
        this.admin_endpoint = fetch(
            `${cdn_domain}/${sreg}/admin-${this.__endpoint_version}.json`
        )
            ...
            .then((data) => {
                try {
                    ...
                    throw new SkapiError(
                        "Service does not exist. Create your service from skapi.com",
                        { code: "NOT_EXISTS" }
                    );
                }
            });

        this.record_endpoint = fetch(
            `${cdn_domain}/${sreg}/record-${this.__endpoint_version}.json`
        )
            .then((response) => response.blob())
            .then(
                (blob) =>
        ...
        const restore = JSON.parse(
            sessionStorage.getItem(`${service}#${owner}`) || "null"
        );
        ...
        this.__authConnection = (async (): Promise<void> => {
            ...
            try {
                let fireWhenAutoLogin = await authentication
                    .bind(this)()
                    .getSession({
                        _holdLogin: true,
                    });
        ...
        this.__connection = (async (): Promise<Connection> => {
            ...
            const storeClassProperties = () => {
                ...
                return connection instanceof Promise
    ...
    private async _updateConnection(): Promise<Connection> {
        try {
            ...
            alert(
                "Service is not available: " + (err.message || err.toString())
            );
    ...
    async version(): Promise<string> {
        ...
        if (this.connection.group === 1) {
            console.log(
                `%cSKAPI: THE SERVICE IS IN TRIAL MODE. ALL THE USERS AND DATA WILL BE INITIALIZED EVERY 30 DAYS.`,
    ...
    registerNewsletterGroup(
        ...
        return registerNewsletterGroup.bind(this)(
            params
        ) as Promise<"SUCCESS: Your newsletter group has been registered.">;
    }
    @formHandler()
    ...
    @formHandler()
    createAccount(
        form: { email: string; password: string } & UserAttributes &
            UserProfilePublicSettings
    ): Promise<
        UserProfile &
            PublicUser & {
    ...
    getUsers(
        ...
        fetchOptions?: FetchOptions
    ): Promise<DatabaseResponse<PublicUser>> {
    ...
    grantPrivateRecordAccess(params: {
        ...
        return grantPrivateRecordAccess.bind(this)(params);
    }
    @formHandler()
    ...
    uploadFiles(
        ...
        return uploadFiles.bind(this)(fileList, params);
    }
    @formHandler()
    mock(
        data: Form<
        ...
        return mock.bind(this)(data, options);
    }
    ...
    login(
        ...
        return login.bind(this)(form);
    }
    @formHandler()
    logout(
        form?: Form<{ global: boolean }>
    ): Promise<"SUCCESS: The user has been logged out."> {
        return logout.bind(this)(form);
    }
    ...
    resetPassword(
        ...
        return resetPassword.bind(this)(form);
    }
    @formHandler({ preventMultipleCalls: true })
    ...
    changePassword(params: {
        ...
        return changePassword.bind(this)(params);
    }
    @formHandler({ preventMultipleCalls: true })
    updateProfile(form: Form<UserAttributes>): Promise<UserProfile> {
        return updateProfile.bind(this)(form);
    }
    @formHandler()
    postRecord(
        ...
        return postRecord.bind(this)(form, config, files);
    }
    @formHandler()
    getSubscriptions(
        params: {
...
export default class Skapi {
	...
	constructor(
		...
		if (options) {
			...
			if (typeof options.requestBatchSize === 'number') {
				if (options.requestBatchSize < 1) {
					throw new SkapiError(
						'"requestBatchSize" must be greater than 0.',
						{ code: 'INVALID_PARAMETER' },
					);
				}
		...
		if (
			...
			typeof options.eventListener === 'object'
		) {
			if (
				options.eventListener?.onLogin &&
				typeof options.eventListener.onLogin === 'function'
			...
			if (
				options.eventListener?.onUserUpdate &&
				typeof options.eventListener.onUserUpdate === 'function'
			) {
				this.onUserUpdate = options.eventListener.onUserUpdate;
			}

			if (
				options.eventListener?.onBatchProcess &&
				typeof options.eventListener.onBatchProcess === 'function'
			) {
		...
		const restore = hasWindow
			? JSON.parse(
					window.sessionStorage.getItem(`${service}#${owner}`) ||
						'null',
				)
			: null;
	...
	async version(): Promise<string> {
		...
		if (isBrowserRuntime()) {
			...
			if (this.connection.group === 1) {
				console.log(
					`%cSKAPI: THE SERVICE IS IN TRIAL MODE.`,
					`font-family: monospace; color:red;`,
				);
			}
		}
	 ...
	 */
	@formHandler()
	getspell(params?: { search?: 'spell' | 'name'; value?: string }): Promise<
		DatabaseResponse<{
			spell: string;
			name: string;
			magic?: any;
	...
	@formHandler()
	logout(
		params?: Form<{ global: boolean }>,
	): Promise<'SUCCESS: The user has been logged out.'> {
		return logout.bind(this)(params);
	}
	...
	resetPassword(
		...
		return resetPassword.bind(this)(params);
	}
	/**
	 ...
	 */
	@formHandler({ preventMultipleCalls: true })
	updateProfile(params: Form<UserAttributes>): Promise<UserProfile> {
		return updateProfile.bind(this)(params);
	}
	/**
