...
export async function normalizeRecord(
    ...
    if (record?.rec) {
        if (_called_from !== "called from postRecord") {
            let recPost = sessionStorage.getItem(
                `${this.service}:post:${record.rec}`
            );
            if (recPost) {
                try {
            ...
            if (Array.isArray(r)) {
                for (let url of r) {
                    ...
                    if (access_group !== "public") {
                        url_endpoint = (await getFile.bind(this)(url, {
                            dataType: "endpoint",
                            _ref,
                        })) as string;
                    }
...
<<<<<<< HEAD
                throw new SkapiError("Value parse error.", {
                    code: "PARSE_ERROR",
                });
=======
...
<<<<<<< HEAD
        throw new SkapiError('"endpoints" should be type: array | string.', {
            code: "INVALID_PARAMETER",
        });
    }

    let updatedRec = await request.bind(this)(
        "del-files",
        {
            endpoints,
            storage: "records",
...
<<<<<<< HEAD
        if (target_key[0] === "auth" || target_key[0] === "publ") {
            try {
                ...
                throw new SkapiError("Invalid file url.", {
                    code: "INVALID_PARAMETER",
                });
=======
        if (target_key[0] === 'auth' || target_key[0] === 'publ') {
...
<<<<<<< HEAD
            throw new SkapiError(
                "Expires option can only be used on skapi cdn endpoints.",
                { code: "INVALID_PARAMETER" }
            );
        }

        if (expires < 0) {
            throw new SkapiError('"config.expires" should be > 0. (seconds)', {
                code: "INVALID_PARAMETER",
            });
        }
        ...
        url = (
            await request.bind(this)("get-signed-url", params, { auth: true })
        ).url;
    } else if (needAuth) {
        ...
        if (this.session.idToken.payload.exp < currTime) {
            ...
            try {
                ...
                throw new SkapiError("User login is required.", {
        ...
        let access_group =
            target_key[6] === "**" ? "**" : parseInt(target_key[6]);

        if (
            this.user.user_id !== target_key[3] &&
            (access_group === "**" || this.user?.access_group < access_group)
        ) {
            let record_id = target_key[5];
            if (
                this.__private_access_key[record_id] &&
                typeof this.__private_access_key[record_id] === "string"
            ) {
                url += "&p=" + this.__private_access_key[record_id];
            } else if (this.owner !== this.host) {
                try {
                    let p = await this.requestPrivateRecordAccessKey({
                        record_id,
                        reference_id: config?._ref,
                    });
                    url += "&p=" + p;
                } catch (err) {}
=======
...
<<<<<<< HEAD
                {
                    method: "get",
                    contentType: null,
                    responseType: config?.dataType === "text" ? "text" : "blob",
                    fetchOptions: { progress: config?.progress },
                },
                { ignoreService: true }
            );
...
<<<<<<< HEAD
        if (typeof is_reference_fetch === "string") {
            if (
                typeof this.__private_access_key?.[is_reference_fetch] ===
                "string"
            ) {
                query.private_key =
                    this.__private_access_key?.[is_reference_fetch] ||
                    undefined;
=======
        if (typeof is_reference_fetch === 'string') {
...
<<<<<<< HEAD
        ...
        query = validator.Params(
            query || {},
            getStruct.bind(this)(query),
            ref_user || isAdmin ? [] : ["table"],
            { ignoreEmpty: true }
        );
    }
    return {
...
<<<<<<< HEAD
    ...
    if (
        is_reference_fetch &&
        result?.reference_private_key &&
        typeof result.reference_private_key === "string"
    ) {
        this.__private_access_key[is_reference_fetch] =
            result.reference_private_key;
    }
...
export async function postRecord(
    ...
    if (!config) {
        throw new SkapiError('"config" argument is required.', {
            code: "INVALID_PARAMETER",
        });
    }
    ...
    let reference_limit_check = (v: number) => {
        if (v === null) {
            ...
            if (0 > v) {
                throw new SkapiError(`"reference_limit" should be >= 0`, {
                    code: "INVALID_PARAMETER",
                });
            }
            ...
            if (v > 4503599627370546) {
                throw new SkapiError(
                    `"reference_limit" should be <= 4503599627370546`,
                    { code: "INVALID_PARAMETER" }
                );
            }
        ...
        throw new SkapiError(
            `"reference_limit" should be type: <number | null>`,
            { code: "INVALID_PARAMETER" }
        );
    };
    ...
    if (config.table?.subscription) {
        if (config.table?.subscription?.is_subscription_record) {
            ...
            config.table?.subscription?.is_subscription_record === false ||
            (!config.record_id &&
                !config.table.subscription?.is_subscription_record)
        ) {
        ...
        record_id: [
            ...
            () => {
                if (!config.table || !config.table.name) {
                    throw new SkapiError('"table.name" is required.', {
                        code: "INVALID_PARAMETER",
                    });
                }
            },
...
<<<<<<< HEAD
                    ...
                    range: (val) => {
                        if (val !== null && typeof v.value !== typeof val) {
                            throw new SkapiError(
                                'Index restriction "range" type should match the type of "value".',
                                { code: "INVALID_PARAMETER" }
                            );
                        }
                        if (!v.hasOwnProperty("value")) {
                            throw new SkapiError(
                                'Index restriction "value" is required.',
                                { code: "INVALID_PARAMETER" }
                            );
                        }
                        if (
                            v.condition &&
                            (v.condition !== "eq" || v.condition !== "=")
                        ) {
                            throw new SkapiError(
                                'Index restriction "condition" cannot be used with "range".',
                                { code: "INVALID_PARAMETER" }
                            );
                        }
                        return val;
...
<<<<<<< HEAD
            if (typeof v !== "object") {
                throw new SkapiError(
                    '"reference" should be type: <string | object>.',
                    { code: "INVALID_PARAMETER" }
                );
            }
...
<<<<<<< HEAD
                    if (typeof this.__private_access_key?.[v] === "string") {
                        config.reference_private_key =
                            this.__private_access_key[v] || undefined;
                    }
                    return validator.specialChars(
                        v,
                        '"reference.record_id"',
                        false,
                        false
            ...
            if (Array.isArray(v)) {
                for (let i of v) {
                    if (typeof i === "string") {
                        ...
                        throw new SkapiError(
                            `"remove_bin" should be type: <string[] | BinaryFile[] | null>`,
                            { code: "INVALID_PARAMETER" }
                        );
                    }
                }
...
<<<<<<< HEAD
        ...
        let { bin_endpoints } = await uploadFiles.bind(this)(
            bin_formData,
            uploadFileParams
        );
        if (!rec.bin) {
...
<<<<<<< HEAD
    if (is_reference_post && typeof rec?.reference_private_key === "string") {
        this.__private_access_key[is_reference_post] =
            rec.reference_private_key;
    }

    sessionStorage.setItem(
        `${this.service}:post:${rec.rec}`,
        JSON.stringify(rec)
    );

    let record = await normalizeRecord.bind(this)(
        rec,
        "called from postRecord"
    );
    if (record.unique_id) {
        this.__my_unique_ids[record.unique_id] = record.record_id;
        sessionStorage.setItem(
            `${this.service}:uniqueids`,
            JSON.stringify(this.__my_unique_ids[record.unique_id])
        );
=======
    if (is_reference_post && typeof rec?.reference_private_key === 'string') {
...
> {
    if (!query?.table) {
        throw new SkapiError('"table" is required.', {
            code: "INVALID_PARAMETER",
        });
    }
    ...
    let p = validator.Params(
        query || {},
        {
            table: "string",
            index: (v: string) =>
                validator.specialChars(v, "index name", true, false),
            order: {
                by: [
                    "average_number",
                    "total_number",
    ...
    if (p.hasOwnProperty("order")) {
        if (!p.order?.by) {
            throw new SkapiError('"order.by" is required.', {
                code: "INVALID_PARAMETER",
            });
        }

        if (
            p.order.hasOwnProperty("condition") &&
            !p.order.hasOwnProperty("value")
        ) {
            throw new SkapiError('"value" is required for "condition".', {
                code: "INVALID_PARAMETER",
            });
        }
        ...
        if (p.hasOwnProperty("index")) {
            if (p.index.substring(p.index.length - 1) !== ".") {
                throw new SkapiError(
                    '"index" should be a parent index name of the compound index when using "order.by"',
                    { code: "INVALID_PARAMETER" }
                );
=======
...
export async function deleteRecords(
    query: DelRecordQuery & { private_key?: string },
    fetchOptions?: FetchOptions
    ...
    let is_reference_fetch = q.is_reference_fetch;
    let result = await request.bind(this)("del-records", q.query, {
        auth: true,
        fetchOptions,
    });
    if (
    ...
    params = validator.Params(
        params,
        {
            ...
            user_id: (v: string | string[]) => {
                if (!v) {
                    throw new SkapiError(`User ID is required.`, {
                        code: "INVALID_PARAMETER",
                    });
                }
...
<<<<<<< HEAD
    if (!params.record_id) {
        throw new SkapiError(`Record ID is required.`, {
            code: "INVALID_PARAMETER",
        });
    }
    ...
    if (
        !params.user_id ||
        (Array.isArray(params.user_id) && !params.user_id.length)
    ) {
        throw new SkapiError(`User ID is required.`, {
            code: "INVALID_PARAMETER",
        });
...
export function requestPrivateRecordAccessKey(params: {
    ...
    if (typeof record_id !== "string") {
        throw new SkapiError(`Record ID should be type: <string | string[]>`, {
            code: "INVALID_PARAMETER",
        });
    }

    if (reference_id && typeof reference_id !== "string") {
        throw new SkapiError(`Reference ID should be type: <string>`, {
            code: "INVALID_PARAMETER",
        });
    }
