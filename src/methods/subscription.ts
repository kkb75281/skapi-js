...
export async function getFeed(
    ...
    params = validator.Params(params || {}, {
        access_group: (v) => {
            ...
            if (typeof v !== "number") {
                throw new SkapiError('"access_group" should be type number.', {
                    code: "INVALID_PARAMETER",
                });
            }
            if (v < 0) {
                throw new SkapiError(
                    '"access_group" should be zero or a positive number.',
                    { code: "INVALID_PARAMETER" }
                );
            }
            if (v > this.__user.access_group) {
                throw new SkapiError("User has no access.", {
    ...
    let recs = await request.bind(this)("get-feed", params, {
        auth: true,
        fetchOptions,
    });
=======
...
<<<<<<< HEAD
        throw new SkapiError(`"user_id" cannot be the user's own ID.`, {
            code: "INVALID_PARAMETER",
        });
=======
...
> {
    ...
    params = validator.Params(params, {
        subscriber: (v: string) =>
            validator.UserId(v, 'User ID in "subscriber"'),
    ...
    if (!params.subscriber && !params.subscription) {
        throw new SkapiError(
            'At least either "subscriber" or "subscription" should have a value.',
            { code: "INVALID_PARAMETER" }
        );
    }

    let response = await request.bind(this)(
        "get-subscription",
        params,
...
export async function blockSubscriber(params: {
    user_id: string;
...
export async function unblockSubscriber(params: {
    user_id: string;
...
> {
    ...
    params = validator.Params(params, {
        ...
        group: (v) => {
            ...
            if (typeof v !== "number") {
                throw new SkapiError(
                    '"group" should be type number | "public" | "authorized".',
                    { code: "INVALID_PARAMETER" }
                );
            }
            return v;
    ...
    let list = await request.bind(this)("get-newsletter-subscription", params, {
        auth: true,
    });
    let result = [];
    for (let sub of list) {
...
<<<<<<< HEAD
            ...
            group: [
                ...
                (v: string) => {
                    if (
                        ...
                        throw new SkapiError(
                            '"group" should be an alphanumeric string without spaces and less than 20 characters.',
                            { code: "INVALID_PARAMETER" }
                        );
                    }
                    return v;
    ...
    return request.bind(this)(
        `subscribe-${this.__user ? "" : "public-"}newsletter`,
        params,
        { auth: !!this.__user }
    );
}
...
export async function registerNewsletterGroup(
        ...
        form || {},
        {
            ...
            restriction: (v: number) => {
                if (typeof v !== "number" || v < 0 || v > 99) {
                    throw new SkapiError(
                        '"restriction" should be a number between 0 and 99.',
                        { code: "INVALID_PARAMETER" }
                    );
                }
                return v;
    ...
    return request.bind(this)("register-newsletter-group", params, {
        auth: true,
    });
}
        ...
        params,
        {
            ...
            value: (v: number | string) => {
                if (typeof v !== searchType[params.searchFor]) {
                    throw new SkapiError(
                        `"value" type does not match the type of "${params.searchFor}" index.`,
                        { code: "INVALID_PARAMETER" }
                    );
            ...
            range: (v: number | string) => {
                if (
                    ...
                    typeof v !== typeof params.value
                ) {
                    throw new SkapiError(
                        '"range" should match type of "value".',
                        { code: "INVALID_PARAMETER" }
                    );
                }
                return v;
            ...
            group: (x: number | string) => {
                ...
                if (!this.session) {
                    throw new SkapiError("User should be logged in.", {
                        code: "INVALID_REQUEST",
                    });
                }
                ...
                if (typeof x === "number") {
                    if (
                        !isAdmin &&
                        x > parseInt(this.session.idToken.payload.access_group)
                    ) {
                        throw new SkapiError("User has no access.", {
                            code: "INVALID_REQUEST",
                        });
        ...
        Object.assign(
            {
                method: "get",
                auth:
                    endpointTarget === "get-public-newsletters"
                        ? !!this.__user
                        : true,
            },
            { fetchOptions }
